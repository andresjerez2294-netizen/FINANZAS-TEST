...
import { 
  ...
  Edit2, Brain, Copy, Target, PiggyBank, Save, Download, Upload, FileJson
} from 'lucide-react';
import { 
...
const GlobalStyles = () => (
  <style>{`
    ...
    /* FECHA NATIVA SIMPLE PARA EVITAR ERRORES EN MOVIL */
    input[type="date"] { color-scheme: dark; background: transparent; color: white; width: 100%; cursor: pointer; }
    ...
    <div className="bg-[#1e293b] p-4 rounded-[2rem] border border-slate-700 shadow-xl space-y-4 lg:sticky lg:top-6">
      ...
      <div>
        ...
        <form onSubmit={onSave} className="space-y-2.5">
          ...
          {/* FECHA ARREGLADA: INPUT NATIVO SIMPLE */}
          <div className="relative w-full bg-slate-900 border border-slate-700 rounded-xl flex items-center px-3 py-2.5 focus-within:border-purple-500">
             <input type="date" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} className="bg-transparent text-white w-full outline-none font-bold text-xs uppercase"/>
             <Calendar size={16} className="text-slate-500 absolute right-3 pointer-events-none"/>
          </div>
        ...
        <div className="bg-[#1e293b] w-full max-w-sm rounded-[2rem] p-5 relative max-h-[80vh] overflow-y-auto scrollbar-hide border border-slate-700 shadow-2xl">
          ...
          <div className="space-y-3">
             ...
             <div className="bg-slate-900 p-3 rounded-xl flex gap-2">
                <button onClick={exportData} className="flex-1 bg-slate-800 py-2 rounded-lg text-[10px] font-bold text-emerald-400 border border-emerald-900/50 flex items-center justify-center gap-1"><Download size={12}/> Copia Seguridad</button>
                <label className="flex-1 bg-slate-800 py-2 rounded-lg text-[10px] font-bold text-blue-400 border border-blue-900/50 flex items-center justify-center gap-1 cursor-pointer">
...
function Dashboard({ user, logout }) {
  ...
  const exportData = async () => {
      try {
          ...
          const trans = transSnap.docs.map(d => ({...d.data(), _id: d.id})); // Keep ID just in case
  ...
  const importData = async (e) => {
      ...
      reader.onload = async (event) => {
         try {
             ...
             // 2. Restore Transactions (Avoid duplicates by checking content roughly or just adding all? Risk of dupes)
             // Simple approach: Add all. User should delete current data first if they want clean restore.
             // Better: Check if ID exists? No, Firestore IDs in backup might conflict if generated.
             // Let's just Add them as new docs to be safe from overwriting other user's data if ID collision (rare).
             if(data.transactions && Array.isArray(data.transactions)) {
                 const batch = writeBatch(db);
                 let count = 0;
                 data.transactions.forEach(t => {
                     // Remove old ID to generate new one, or keep it? 
                     // Let's remove _id and force new one to be safe.
                     const { _id, ...transData } = t; 
                     const newRef = doc(collection(db, "transactions"));
                     batch.set(newRef, { ...transData, uid: user.uid }); // Ensure UID is current user
                     count++;
                 });
                 await batch.commit();
